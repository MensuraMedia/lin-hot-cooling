...
| Source | [TECHNICAL-CONCEPT.md](../TECHNICAL-CONCEPT.md) v0.2 (section numbers below refer to it) |
...
| M3.5 | Rule engine | Triggers (AC/battery, GameMode, process class, temperature thresholds, utilization windows, lid, schedule), priorities, hysteresis, dwell (§7.4) |
| M3.6 | Session agent | `lin-hot-cooling --agent` as a systemd **user** unit, so rules run without the window |
| M3.7 | Pages | Templates (catalog grid, detail with resolved targets, clone/edit with form + YAML), Automation (rules, test trigger, event history), a full "Why" feed |
